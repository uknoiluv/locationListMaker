...
describe("directives.api.map", function() {
  beforeEach(function() {
    return window['Initiator'.ns()].initDirective(this, "Map", ['initAll'], {});
  });
  afterEach(function() {
    return this.log.error.calls.reset();
  });
  return it("it should delay creation until center is set", function() {
    var element, html;
    html = angular.element("<ui-gmap-google-map center=\"map.center\" control=\"map.control\" zoom=\"map.zoom\">\n</ui-gmap-google-map>");
    this.scope.map.control = {};
    element = this.compile(html)(this.scope);
    this.rootScope.$apply();
    expect(this.scope.map.control.getGMap).toBeUndefined();
    this.scope.map.zoom = 10;
